...
def main(data_directory, extra_directory):
    ...
    # example: python3 extract_features_from_properties_training.py /Users/luoyisang/FP/fp-inspector-classification/output/data /Users/luoyisang/FP/fp-inspector-classification/extra_data
    # data_directory = sys.argv[1]
    # extra_directory = sys.argv[2]

    # assume this file is present , copy this file to ec2 instance

    # given, maping between feature and names
    dynamic_feature_mapping_file = os.path.join(
        extra_directory, 'dynamic_feature_mapping.json')

    # given
    js_api_count_file = os.path.join(
        extra_directory, 'maximum_number_of_api_calls.json')
    ...
    updated_features_csv = os.path.join(
        data_directory, 'updated_dynamic_features.csv')
    arff_addr = os.path.join(data_directory, 'features_with_encoding.arff')
    # alphabet all convert to numeric value
    feature_mapping_csv = os.path.join(
        data_directory, 'dynamic_features_with_mappings.csv')
    # alphabet all convert to empty???
    feature_csv_encoding = os.path.join(
        data_directory, 'features_with_encoding.csv')
    features_integers_mapping = os.path.join(
        extra_directory, 'features_integers_mapping.json')
    ...
    # NEED TO RUN
    feature_mapping = utilities.read_json(
        dynamic_feature_mapping_file)  # just read from file here
    global_symbols_count = utilities.read_json(
        js_api_count_file)  # just read from file here

    # generate url_index_mapping('url_index_mapping.txt') and features_csv'dynamic_features.csv',
    construct_features(data_directory, feature_mapping,
                       global_symbols_count, url_index_mapping, features_csv)
  
    print(updated_features_csv)
    remove_unwanted_characters_from_csv(features_csv, updated_features_csv)
    
    organize_features(updated_features_csv,
                      feature_mapping_csv, features_integers_mapping)
    one_hot_encoder(feature_mapping_csv, feature_csv_encoding,
                    features_integers_mapping)
    convert_to_arff(feature_csv_encoding, arff_addr, features_integers_mapping)


if __name__ == '__main__':
    # main()
    main(data_directory, extra_directory)
