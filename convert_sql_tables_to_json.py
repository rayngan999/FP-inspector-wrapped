...
def convert_to_json(db_addr, data_directory, file_name, partition_size=10):
    ...
    for row in cur:
        ...
        if str_id not in id_key_map:
            if next_partition_index != current_partition_index:
                ...
                utilities.write_json(os.path.join(data_directory, file_name + '_partition_' + str(
                    current_partition_index) + '.json'), js_data, indent_length=0)
                if os.path.exists(os.path.join(data_directory, file_name + '_partition_' + str(next_partition_index) + '.json')):
                    js_data = utilities.read_json(os.path.join(
                        data_directory, file_name + '_partition_' + str(next_partition_index) + '.json'))
                    print('39: Writing:', file_name + '_partition_' + str(current_partition_index), 'Reading:',
                          file_name + '_partition_' + str(next_partition_index), 'Next Partition: ', next_partition_index)
                else:
                    print('39: Writing:', file_name + '_partition_' + str(current_partition_index), 'Not Reading:',
                          file_name + '_partition_' + str(next_partition_index), 'Next Partition: ', next_partition_index)
            ...
            if required_partition_index != current_partition_index:
                utilities.write_json(os.path.join(data_directory, file_name + '_partition_' + str(
                    current_partition_index) + '.json'), js_data, indent_length=0)
                js_data = utilities.read_json(os.path.join(
                    data_directory, file_name + '_partition_' + str(required_partition_index) + '.json'))
                print('58: Writing:', file_name + '_partition_' + str(current_partition_index), 'Reading:',
                      file_name + '_partition_' + str(required_partition_index), 'Next Partition: ', next_partition_index)
        ...
        js_data[str_id_index]['information'].append(
            {'symbol': row[3], 'argument': row[4], 'value': row[5]})
        ...
        if id_index % partition_size == 0:
            ...
            if next_check:
                utilities.write_json(os.path.join(data_directory, file_name + '_partition_' + str(
                    next_partition_index) + '.json'), js_data, indent_length=0)
                next_partition_index += 1
                print('76: Writing:', file_name + '_partition_' +
                      str(current_partition_index), 'Next Partition: ', next_partition_index)
                current_partition_index = next_partition_index
    ...
    utilities.write_json(os.path.join(
        data_directory, 'mapping.json'), id_key_map, indent_length=4)
    return




def convert_to_json_greedy(db_addr, data_directory, file_name, partition_size=10):
    con = sqlite3.connect(db_addr)
    con.row_factory = sqlite3.Row
    cur = con.cursor()
    cur.execute("SELECT MAX(id) as max_id FROM javascript")
    ...
    for row in cur:
        ...
        if str_id not in id_key_map:
            ...
            if required_partition_index != current_partition_index:
                ...
                remaining_js_data[required_partition_index][str_id_index]['information'].append(
                    {'symbol': row[3], 'argument': row[4], 'value': row[5]})
        ...
        if id_index % partition_size == 0:
            ...
            if check_status:
                utilities.write_json(os.path.join(data_directory, file_name + '_partition_' + str(
                    current_partition_index) + '.json'), js_data, indent_length=0)
                print('76: Writing:', file_name + '_partition_' + str(current_partition_index),
    ...
    utilities.write_json(os.path.join(data_directory, file_name + '_partition_' +
                                      str(current_partition_index) + '.json'), js_data, indent_length=0)
    utilities.write_json(os.path.join(
        data_directory, 'remaining.json'), remaining_js_data, indent_length=0)
    utilities.write_json(os.path.join(
        data_directory, 'mapping.json'), id_key_map, indent_length=4)
...
def adjust_greedy(partitioned_data_directory, partitioned_file_name='javascript_partition_', remaining_file_name='remaining.json'):
    remaining_json = utilities.read_json(os.path.join(
        partitioned_data_directory, remaining_file_name))

    pbar_overall = tqdm(total=len(remaining_json))
    for key in remaining_json:
        current_json = utilities.read_json(os.path.join(
            partitioned_data_directory, partitioned_file_name + key + '.json'))
        ...
        utilities.write_json(os.path.join(partitioned_data_directory,
                                          partitioned_file_name + key + '_updated.json'), current_json, indent_length=0)
...
def update_hash(db_addr, partitioned_data_directory, mapping_file_name='mapping.json'):
    mapping_json = utilities.read_json(os.path.join(
        partitioned_data_directory, mapping_file_name))
    ...
    for row in cur:
        if row[2] != '' and row[2] != None:
            ...
            if row[1].strip() != '' and row[1].strip() in url_updated_mapping_object:
                url_updated_mapping_object[row[1].strip(
                )]['content_hash'] = row[2].strip()
                total_keys_matched += 1
    ...
    utilities.write_json(os.path.join(partitioned_data_directory,
                                      'mapping_updated.json'), updated_mapping_object, indent_length=0)
    utilities.write_json(os.path.join(partitioned_data_directory,
                                      'url_only_mapping_updated.json'), url_updated_mapping_object, indent_length=0)

    for key in mapping_json:
        mapping_json[key]['content_hash'] = updated_mapping_object[mapping_json[key]
                                                                   ['new_id_url_key']]['content_hash']
        if mapping_json[key]['content_hash'] == '':
            mapping_json[key]['content_hash'] = url_updated_mapping_object[mapping_json[key]
                                                                           ['new_url_key']]['content_hash']

    utilities.write_json(os.path.join(partitioned_data_directory,
                                      'mapping_with_hash.json'), mapping_json, indent_length=0)
    return
...
def update_hash_in_partitioned_files(data_directory, file_name, suffix_1='_partition_', suffix_2='_updated', mapping_file_with_hash='mapping_with_hash.json'):
    ...
    for key in mapping_json:
        ...
        dict_per_partition[mapping_json[key]['partition_index']][mapping_json[key]
    ...
    for key in dict_per_partition:
        ...
        if key == len(dict_per_partition):
            current_json = utilities.read_json(os.path.join(
            data_directory, file_name + suffix_1 + str(key) + '.json'))
        else:
            current_json = utilities.read_json(os.path.join(
            data_directory, file_name + suffix_1 + str(key) + suffix_2 + '.json'))
        for item in dict_per_partition[key]:
            current_json[item]['content_hash'] = dict_per_partition[key][item]['content_hash']
        utilities.write_json(os.path.join(data_directory, file_name +
                                          '_with_hash_partition_' + str(key) + '.json'), current_json)
        pbar.update(1)
    return

# python3 dynamic_analysis.py "./" "./crawl-data_part3.sqlite" "./output" "./data" 5000 
def main(db):
    # example python3 convert_sql_tables_to_json.py "/Users/luoyisang/FP/fp-inspector-classification" "crawl-data.sqlite" "output" "data" 10
    # print(db)
    db_addr = db
    ...
    convert_to_json_greedy(db_addr, data_directory,
                           file_name, partition_size=input_partition_size)
