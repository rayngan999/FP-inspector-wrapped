...
def expand_params(url):
    purl = urlparse(url)
    print("\n====================================")
    print("Scheme: %s" % purl.scheme)
    print("Hostname: %s" % purl.hostname)
    print("Path: %s" % purl.path)
...
def read_cache_or_query_db(con, db_query, csv_file="", versioned=True):
    ...
    if csv_file != "" and versioned:
        ...
        csv_file = "%s-%s.csv" % (parts[0], hashlib.md5(db_query).hexdigest())
    used_cache = False
...
def list_placeholder(length, is_pg=False):
    ...
    if is_pg, returns (%s,%s,%s,....) instead.
    """
    if is_pg:
        return '(' + '%s,'*(length-1) + '%s)'
    else:
...
def insert_get_id(cursor, table, arguments, unique):
    ...
    if not insert:
        ...
        cursor.execute("SELECT id FROM %s WHERE %s" % (
            table, " AND ".join(map(lambda x: "%s = ?" % x, unique))),
            map(lambda x: arguments[x], unique))
...
def insert_ignore(cursor, table, arguments, unique, index_fn=None):
    ...
    if index_fn is not None:
        where_str = index_fn + "(%s) = " + index_fn + "(%%s)"
    else:
        where_str = "%s = %%s"
    ...
    query = "".join((  # makes substitutions more readable
        "WITH s AS ( ",
            "SELECT id ",
            "FROM %s " % table,
            "WHERE %s " % " AND ".join(map(lambda x: where_str % x, unique)),
        "), i AS ( ",
            "INSERT INTO %s (%s) " % (table, ','.join(arguments.keys())),
            "SELECT %s " % ",".join(["%s"]*len(arguments)),
...
def build_page_source_df(src_dir, visit_ids=None, include_iframes=False):
    ...
    if not os.path.isdir(src_dir):
        raise ValueError("%s not found" % src_dir)

    src_zips = glob.glob(os.path.join(src_dir, '*.json.gz'))

    if len(src_zips) == 0:
        raise ValueError("%s contains no source files" % src_dir)
...
def get_page_source(src_dir, visit_id, include_iframes=False):
    ...
    src_zips = glob.glob(os.path.join(src_dir, '%s-*.json.gz' % visit_id))

    if len(src_zips) == 0:
        raise ValueError("%s contains no source files from visit_id %s" %
                         (src_dir, visit_id))
